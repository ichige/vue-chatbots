...
RAG となるデータベースさせ用意できていれば、LlamaIndex の提供している高レベルAPIにて実装すれば、そこそこ満足する仕上がりになるような気がします。  
...
まずは tool calling の関数 `ask_user` はいきなり例外を投げます。
...
このアプリにおいて、LLMとのチャットの内容は単純ではあるのですが、ユーザへの質問を tool calling 経由で実行させる事、履歴からすでに質問した内容を読み取らせて「質問への回答済」であることを理解させる事の２点に関しては、低レベルのモデルでは不可能なようでした。  
gemini-3.1-flash-lite-preview 以上で、ようやく安定して動作します。それでも時々質問を tool calling ではなくチャットのレスポンスとして返すこともしばしば発生するので、本番運用？するのであれば、そこそこのモデルを選択しつつ、保険として回答結果がそれらしいものかどうかを再度AIに問い合わせるなど、より繊細な処理フリーも必要になるかと思います。
...
もう少し高度なアプリを作成する場合は、ステート管理クラスを用意して、ステート(フラグ)に対して1つの動作が動き、処理が完了したら別のステートへ遷移させるという流れを作れば、カオスにならずに済みそうな感じがしました。
