...
- LlamaIndex の処理フローを強制的に停止させること。
- そもそも tool calling がチャットではないため履歴管理の対象にならない。
- 結果として自前で履歴管理しつつ、AI に質問内容とその解答を伝える必要が生じる。

まず第1に、tool calling の途中で のAI エージェントのフローを強制停止するのは簡単ではありません。  
...
このアプリにおいて、LLMとのチャットの内容は単純ではあるのですが、ユーザへの質問を tool calling 経由で実行させる事、履歴からすでに質問した内容を読み取らせて「質問への回答済」であることを理解させる事の２点に関しては、軽量モデルでは不可能なようでした。  
...
もう少し高度なアプリを作成する場合は、ステート管理クラスを用意して、ステート(フラグ)に対して1つの動作が動き、処理が完了したら別のステートへ遷移させるという流れ(ステートマシン)を作れば、ある程度の保守性を担保出来そうな気がします。

現状は issues も多いようですが、多くのものは追加機能への期待の現れでもあり、ツールの需要の多さにも比例しているのだと思います。
web socket を使っているのであれば、Human in the loop へのネイティブな対応を期待したいところです。  
